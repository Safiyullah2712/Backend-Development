...
public interface MessageRepository
    ...
    @Query("""
    		SELECT new com.student.management.dto.MessageSearchDTO(
    		m.id,
    		m.content,
    		m.sender.username,
    		m.timestamp
    		...
    		OR
    		LOWER(m.sender.username)
    		LIKE LOWER(CONCAT('%',:keyword,'%'))
    		ORDER BY m.timestamp DESC
    		""")
    		Page<MessageSearchDTO> searchMessages(
    		        String keyword,
    		        Pageable pageable);
    ...
    @Query("""
    		...
    		m.timestamp BETWEEN :fromDate
    		AND :toDate

    		ORDER BY m.timestamp DESC
    		""")
    		Page<MessageSearchDTO> searchMessagesByDate(
    		        String keyword,
